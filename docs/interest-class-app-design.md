...
这个 App 的目标不是做一个复杂的培训机构管理系统，而是做一个家长手机端自用的、数据在自己手上的课外班记录工具。
...
| ------------- | ---------------------------------------------------------------------------------- |
| id            | 成就记录的唯一标识，系统内部使用。                                                 |
| child_id      | 关联的孩子 id，表示这个成就属于哪个孩子。                                          |
| kid_course_id | 关联的孩子课程 id，可选；如果成就与具体课程有关，就填写。                          |
| achieved_at   | 成就发生日期，例如获奖日期、考级日期、演出日期。                                   |
| title         | 成就标题，例如“篮球比赛二等奖”“钢琴三级通过”。                                     |
| type          | 成就类型，普通分类标签；例如获奖、考级、比赛、演出、作品、其他，可新增自定义类型。 |
| description   | 详细描述，可选；记录比赛名称、成绩、老师评价等。                                   |
| created_at    | 创建时间，记录这条成就什么时候创建。                                               |
| updated_at    | 更新时间，记录这条成就最后一次修改时间。                                           |
